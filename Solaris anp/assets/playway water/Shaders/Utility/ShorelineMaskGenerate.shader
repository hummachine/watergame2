...
Shader "PlayWay Water/Utility/ShorelineMaskGenerate"
{
	...
	CGINCLUDE
		...
		float4 copyHeightMap(v2f i) : SV_Target
		{
			...
			//if (i.uv_heightMap.x < 0 || i.uv_heightMap.y < 0 || i.uv_heightMap.x > 1 || i.uv_heightMap.y > 1 || horg >= 0.1)
				h = 0;
				//h = max(0, -_TerrainMinPoint);
