...
Shader "PlayWay Water/Spray/Generator (Local)"
{
	...
	fixed4 frag(VertexOutput vo) : SV_Target
	{
		...
		half shoreMask = tex2D(_LocalSlopeMap, half2(vo.uv_b.x, vo.uv_a.y)).a;
		...
		//spawnRate += tex2D(_LocalSlopeMap, half2(vo.uv_b.x, vo.uv_a.y)).b * 0.1;
		...
		if (r * spawnRate > pow(_Params.y, 1024 / _TotalDisplacementMap_TexelSize.z))
		{
			...
			for (int i = 0; i < _Iterations; ++i)
			{
				...
				float2 worldPos = float2(vo.uv_b.x, vo.uv_a.y) * _LocalMapsCoords.ww - _LocalMapsCoords.xy;
